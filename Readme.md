...
## 확률 분포(Distribution)
 ...
 - 확률 변수가 어떤 종류의 값을 가지는 가에 따라 이산 확률 분포와 연속 확률 분포 중 하나에 속한다.
 - 확률 분포간의 관계도  
  ![](/data/image/Distribution.jpg)

<br>

### 이산 확률 분포(Discreate Probability Distribution)
  - 이산 확률 변수가 가지는 확률 분포를 의마한다.
  - 확률 변수가 가질 수 있는 값의 개수가 셀 수 있는 만큼 존재한다는 의미이다.
  - 사용사례
    - 확률 질량 함수
    - 누적 분포 함수
    - 이산 균등 분포
    ...
    - 이항 분포
    - 다항 분포
    > 해당 함수들은 비약적 불연속으로만 증가한다.  

<br>

### 연속 확률 분포(Continuous Probability Distribution)
 - 확률 밀도 함수를 이용해 분포를 표현할 수 있는 경우를 의미한다.
 - 연속 확률 분포를 가지는 확률 변수는 연속 확률 변수라고 부른다.
 - 사용사례
   - 정규 분포
   - 연속균등 분포
   - 카이제곱 분포
...
## 결측치 처리
 - 데이터에 값이 없는 것을 결측치라고 한다.
 - 결측치를 다 제거하면 막대한 데이터 손실을 부를 수 있다.
 - 결측치를 잘못 대체하면 데이터에서 편향이 생길 수 있다.
 - 결측치 처리에 분석가의 견해가 가장 많이 반영되고, 분석결과가 매우 틀어질 수 있다.
   > 자신의 주관적인 생각이 아닌, 데이터에 기반한 결측치 처리가 진행되어야 정확한 분석을 할 수 있다.  
 
 <br>

### 결측치 처리 전략
 - 제거
   - 결측치의 특성이 무작위로 손실되지 않는다면, 제거하는 것이 가장 좋은 방법이다.
   - 방식
     - 목록삭제
       - 결측치가 있는 행 전부 제거
         > `df.dropna()`  
     ...
     - 단일값 삭제
       ...
       - 행의 결측치가 2개 초과인 경우
         > `df.dropna(thresh = 2)`  
       - 특정 열들 중에 결측치가 있을 경우 해당 행 제거
         > `df.dropna(subset = ['col1', 'col2', 'col3'])`  
         > `df.dropna(subset = ['col1', 'col2', 'col3'], how = 'all')`  
         > `df.dropna(subset = ['col1', 'col2', 'col3'], thresh = 1)`  
   - 사용 함수
     - `dropna()` : pandas에서 제공하는 Na/NaN과 같은 누락 데이터 제거한다.
 - 대체
   - 결측치를 특정 값으로 대체하는 방법이다.
   - 대체 값의 종류
     - 최빈값
       - 범주형에서 결측값이 발생 시, 범주별 빈도가 가장 높은 값으로 대체하는 방법이다.
     - 중앙값
       - 숫자(연속)형에서 결측값을 제외한 중앙값을 대체하는 방법이다.
     - 평균값
       - 숫자(연속)형에서 결측값을 제외한 평균으로 대체하는 방법이다.
     - 유사한 경우 대체
       - 조건 부 대체
     - 일반화된 대체
       - 회귀 분석을 이용한 대체
     - 사용 함수
       - pandas에서 제공하는 누락 데이터에 특정 값을 채우는 함수들이다.
       - `filna()` : 전체 결측치를 특정 단일값으로 대체한다.
         > `df.fillna(0)`  
         - 특정 열에 결측치가 있을 경우 다른 값으로 대체
         > `df['col'].fillna(0)`  
       ...
       - `replace()` : 전체 결측치를 특정 단일값으로 대체한다.
         > `df.replace(to_replace = np.nan, value = -50)`  
       - `interpolate()` : index를 무시하고 값들을 선형적으로 같은 간격으로 처리하게 된다.
         > `df.interpolate(method = 'linear', limit_direction = 'forward')`  
 - 예측 모델 
   - 예측 기법을 사용한 결측치 추정은 결측치들의 특성이 패턴을 가진다고 가정하고 진행한다.
   - 결측값이 없는 column들로 구성된 dataset으로 결측값이 있는 column를 예측한다.
   - 회귀 분석 기술을 활용하거나 SVM과 같은 기계 학습 방법 or 결측치를 채우는 데이터 마이닝 방법과 같은 다양한 통계 방법이 있다.  
